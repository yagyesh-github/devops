...
<c:if test="${empty aemail}">
	...
	<title>Sporty Shoes General Store | Pending Orders </title>
...
<h3 class="text-center">Pending Orders</h3>
	<table id="example" class="table table-hover table-bordered text-center">
          ...
          <br>${orders.product.name}</td>
							...
							<td><a href="${pageContext.request.contextPath}/admin/order/changeStatus/${orderStatus}/${orders.id}" class="btn btn-success" onclick="return confirm('Change Order Status to Delivered?');">Deliver</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
