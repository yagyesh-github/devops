...
<c:if test="${empty aemail}">
	...
	<title>Sporty Shoes General Store | Delivered Orders </title>
...
<h3 class="text-center">Delivered Orders</h3>
	<table id="example" class="table table-hover table-bordered text-center">
				...
				<c:set var="orderStatus" value="Pending" scope="page"></c:set>
					<c:forEach var="orders" items="${delivered}">
          ...
          <br>${orders.product.name}</td>
							...
							<td>${orders.amount}</td>
							<td><span class="badge badge-success">${orders.orderStatus}</span></td>
							<td><fmt:formatDate pattern="dd-MMM-yyyy hh:mm:ss" value="${orders.order.orderDate}" /></td>
							<td><a href="${pageContext.request.contextPath}/admin/order/changeStatus/${orderStatus}/${orders.id}" class="btn btn-warning" onclick="return confirm('Change Order Status to Pending?');">Pending</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
