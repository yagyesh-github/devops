...
<c:if test="${empty aemail}">
	...
	<title>Sporty Shoes General Store | All Orders </title>
...
<h3 class="text-center">All Orders</h3>
	<table id="example" class="table table-hover table-bordered text-center">
		...
		<td>
		<c:if test="${orders.orderStatus eq 'Cancelled'}">
		<a href="${pageContext.request.contextPath}/admin/order/changeStatus/Pending/${orders.id}" class="btn btn-warning" onclick="return confirm('Change Order Status to Pending?');">Pending</a>
		<hr><a href="${pageContext.request.contextPath}/admin/order/changeStatus/Delivered/${orders.id}" class="btn btn-success" onclick="return confirm('Change Order Status to Delivered?');">Deliver</a>
		</c:if>
		<c:if test="${orders.orderStatus eq 'Delivered'}">
		<a href="${pageContext.request.contextPath}/admin/order/changeStatus/Pending/${orders.id}" class="btn btn-warning" onclick="return confirm('Change Order Status to Pending?');">Pending</a>
		</c:if>
