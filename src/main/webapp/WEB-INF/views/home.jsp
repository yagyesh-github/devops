...
<c:if test="${empty aemail}">
	...
	<title>Sporty Shoes General Store | Admin Home </title>
...
<h3 class="text-center">Top 10 Pending Orders</h3>
	<table id="example" class="table table-hover table-bordered text-center">
          ...
          <br>${orders.product.name}</td>
							...
							<td>${orders.amount}</td>
							<td><span class="badge badge-warning">${orders.orderStatus}</span></td>
							<td><fmt:formatDate pattern="dd-MMM-yyyy hh:mm:ss" value="${orders.order.orderDate}" /></td>
							<td><a href="${pageContext.request.contextPath}/admin/order/changeStatus/${orderStatus}/${orders.id}" class="btn btn-success" onclick="return confirm('Change Order Status to Delivered?');">Deliver</a>
<%-- 							<hr> <a href="${pageContext.request.contextPath}/admin/order/changeStatus/Cancelled/${orders.id}" class="btn btn-info" onclick="return confirm('Are you sure you want to cancel this order?');">Cancel</a> --%>
							</td>
						</tr>
					</c:forEach>
				</tbody>
